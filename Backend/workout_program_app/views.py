...
class All_programs(User_permissions):
    def get(self, request):
        programs = UserWorkoutProgramSerializer(request.user.user_programs.order_by("program_id"), many=True)
        return Response(programs.data)

    def post(self, request):
        user = get_object_or_404(App_user, pk=request.user.pk)
        program_id = request.data.get("program_id")
        existing_program = user.user_programs.filter(program_id=program_id).first()
